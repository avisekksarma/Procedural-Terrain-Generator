...
void Terrain::generateInitialTerrain(int _x, int _z, int _w, int _l)
{
    ...
    // debug
    cout << "=======" << endl;
    cout << consts.lengthZ << endl;
    cout << consts.ztrilen << endl;
    cout << consts.widthX << endl;
    cout << consts.xtrilen << endl;
    cout << row << endl;
    cout << col << endl;
    cout << "=======" << endl;
    ...
    for (int i = 0; i < row; i++)
    {
        for (int j = 0; j < col; j++)
        {
            ...
            if (i == 0 and j == 0)
            {
                lC = glMath::returnLocalCoords<float>(glMath::vec3f(x, z, 0.0f), true);
                cout<<lC<<endl;
            }
            else
            {
            ...
            up.p[1] = glMath::vec3f(lC.x, 0.0f, lC.y);
            up.p[2] = glMath::vec3f(lC.x + incX, 0.0f, lC.y);
            up.p[0] = glMath::vec3f(lC.x, 0.0f, lC.y - incZ);

            down.p[0] = up.p[0];
            down.p[1] = up.p[2];
            down.p[2] = glMath::vec3f(lC.x + incX, 0.0f, lC.z - incZ);
